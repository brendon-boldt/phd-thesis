...
\end{enumerate}
\drm{I feel like an important question is ``how can we tell if emergent languages will serve these stated goals and how we can change environments so that they do?'' Both specifically and generally. Being human-like is one meta-criterion.}
...
I do not plan to retain the science--engineering distinction because I think that stricter taxonomy was primarily serving to set up the recommendations of different methods; instead, I plan to make similar but more general distinction between knowledge-driven goals and task-driven goals. \drm{I think this distinction is fine, but that you should be careful of creating too strict a dichotomy.}
...
\drm{This sounds like a good paper.}
...
\drm{I think this also sounds like a good, and ambitious, paper.}
...
This will be an interdisciplinary work which attempts to fit emergent language into the paradigm of simulation-based investigation already present in the aforementioned fields.\drm{Interesting. From my perspective, human languages have not only to be biologically plausible but also socially plausible. That is, languages are like they are not just because they can be used by human brains but because they have to serve as a social instrument.}
...
Language modeling and parsing would serve as intermediate tasks, in this sense. \drm{This is a good paper idea, but one that will require a lot of research. In particular, I think that it is not obvious that human-like languages will produce the best pretraining data (or that these two things should be conflated).}
...
We can use a collaborative Minecraft building task as a possible data source as we have human-human data for this task and it would be possible to train computer agents to complete this task using emergent language.\drm{This is a fun idea. I had actually though about a Minecraft-like environment for other EL experiments, but it is especially appropriate for this one.}
...
Thus, we can generate synthetic languages via context-free grammars of varying complexities and compare the effect of pretraining on performance in a handful of tasks to that provided by emergent languages.\drm{Interesting experiments. Sounds more achievable that sound of the other studies.}
...
The hypothesis here is that the emergent language will very concrete and low-level, where utterances correspond very closely to exactly what block is placed where in the environment.\drm{Good use of the Minecraft env once you have it set up.}
...
Alternatively, it could also mean that the structure of the neural networks generating the emergent language operate significantly different from how the human brain uses language. \drm{This is a paper to which I could really contribute and which would serve to broaden the interdisciplinary scope of the dissertation. I would be very sad if you do not write this paper.}
...
Going beyond purely intrinsic summary, the addition of extrinsic metrics should give a more robust, real-world sense of how sophisticated an emergent language is. \drm{It is not clear to me how to differentiate this paper from a couple of the other papers (on typology and metrics).}
...
Performing this task of predicting the extrinsic based on the intrinsic helps to bridge a fundamental gap which the intrinsic/extrinsic divide presents:
    intrinsic metrics represent a direct understanding of some property of the language but are narrow and might miss relevant details while extrinsic methods are sensitive to a broad range of properties of the language yet are black boxes and difficult to understand by themselves. \drm{I feel like this raises points that will have to be addressed in some of your other papers. I'm not sure it has to be a separate paper/chapter.}


\section{NSF grant proposal notes}
...
\end{itemize}
\cmg{give a more detailed illustration of how one would run this benchmark}
\cmg{explain the purpose and impact, briefly}
...
\drm{For NSF, we need something that is both more focused and more clearly innovative. Let's talk about this.}
